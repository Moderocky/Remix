...
public class SymbolMultiply implements Operator, Element {
    @Override
    ...
    public boolean matches(Context context, String string) {
        ...
        if (context.check() == null) {
            context.error("Multiply is a binary operator.");
            return false;
        }
        return true;
    }
    ...
    public Consumer<MethodVisitor> instruction(Context context, String string) {
        return TypeBuilder.MUL.write();
    }
    
    @Override
    public TypeStub result() {
        return TypeBuilder.MUL.result();
    }
}
