...
public class SymbolDivide implements Operator, Element {
    @Override
    ...
    public boolean matches(Context context, String string) {
        ...
        if (context.check() == null) {
            context.error("Divide is a binary operator.");
            return false;
        }
        return true;
    }
    ...
    public Consumer<MethodVisitor> instruction(Context context, String string) {
        return TypeBuilder.DIV.write();
    }
    
    @Override
    public TypeStub result() {
        return TypeBuilder.DIV.result();
    }
}
