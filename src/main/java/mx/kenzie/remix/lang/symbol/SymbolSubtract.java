...
public class SymbolSubtract implements Operator, Element {
    @Override
    ...
    public boolean matches(Context context, String string) {
        ...
        if (context.check() == null) {
            context.error("Subtract is a binary operator.");
            return false;
        }
        return true;
    }
    ...
    public Consumer<MethodVisitor> instruction(Context context, String string) {
        return TypeBuilder.SUB.write();
    }
    
    @Override
    public TypeStub result() {
        return TypeBuilder.SUB.result();
    }
}
